import unittest
import numpy as np
from euclid.FreudShape import ConvexPolyhedron, ConvexSpheropolyhedron, quatrot
...
class TestShape(unittest.TestCase):
    ...
    def test_quatrot(self):
        # Check quatrot
        z = np.array([0., 0., 1.])
        for i in range(cubepoly.nfacets):
            n = cubepoly.equations[0, 0:3]
            if abs(n[2]) < 1 - tolerance:
                theta = np.arccos(n[2])
                cp = np.cross(n, z)
                k = cp / np.sqrt(np.dot(cp, cp))
                q = np.concatenate(([np.cos(theta/2.)], np.sin(theta/2.) * k))
                # Check
                r = quatrot(q, n)
                assert r[2] >= 1 - tolerance, \
                    'quatrot of {r} by {q} does not align with z'.format(
                        r=r, q=q)
