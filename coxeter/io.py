...
def to_off(shape, filename):
    ...
    with open(filename, "w") as file:
        ...
        file.write(f"{len(shape.vertices)} f{len(shape.faces)} "
                f"{len(shape.edges)}\n")
...
def to_x3d(shape, filename):
    ...
    x3d_shape = ET.SubElement(x3d_scene, "shape", 
                            attrib={"DEF": f"{shape.__class__.__name__}"})
    ...
    coordinate_points = [
        v for f in shape.faces for i in f for v in shape.vertices[i]
    ]
