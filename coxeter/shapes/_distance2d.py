...
def get_vert_zones(shape):
    ...
    vert_constraint = np.append(
        ...
        * np.expand_dims(
            np.append(
                np.expand_dims(shape.edge_vectors[-1], axis=0),
                shape.edge_vectors[:-1],
                axis=0,
            ),
            axis=1,
        ),
...
def get_edge_zones(shape):
    ...
    edge_constraint = np.append(
        -1 * np.expand_dims(shape.edge_vectors, axis=1),
        np.expand_dims(shape.edge_vectors, axis=1),
        axis=1,
    )
...
def get_face_zones(shape):
    ...
    if not np.all(
        ...
        tri_edges = (
            np.append(
                triangle_verts[:, 1:],
                np.expand_dims(triangle_verts[:, 0], axis=1),
                axis=1,
            )
            - triangle_verts
        ...
        # --Polygon is convex--
        face_constraint = np.expand_dims(
            face_constraint, axis=0
        )  # shape = (1, n_edges, 3)
...
def shortest_distance_to_surface(
    ...
    if (
        translation_vector.shape[0] > 3
        or len(translation_vector.shape) > 1
    ...
    vert_bool = np.all(
        (shape.vertex_zones["constraint"] @ points_trans)
        <= np.expand_dims(vert_bounds, axis=2),
        axis=1,
    )  # <--- shape = (number_of_vertex_zones, number_of_points)
    if np.any(vert_bool):
        # v--- shape = (number of True in vert_bool,) ---v
        vert_used = np.transpose(
            np.tile(np.arange(0, shape.num_vertices, 1), (n_points, 1))
        )[
            vert_bool
        ]  # Contains the indices of the vertices that hold True for vert_bool
        v_points_used = np.tile(np.arange(0, n_points, 1), (shape.num_vertices, 1))[
            vert_bool
        ]  # Contains the indices of the points that hold True for vert_bool

        vert_dist = np.ones((shape.num_vertices, n_points)) * max_value
        vert_dist[vert_bool] = np.linalg.norm(
    ...
    edge_bool = np.all(
        ...
        <= np.expand_dims(edge_bounds, axis=2),
        axis=1,
    )  # <--- shape = (number_of_edge_zones, number_of_points)
    if np.any(edge_bool):
        # v--- shape = (number of True in edge_bool,) ---v
        edge_used = np.transpose(
            np.tile(np.arange(0, shape.num_vertices, 1), (n_points, 1))
        )[edge_bool]  # Contains the indices of the edges that hold True for edge_bool
        e_points_used = np.tile(np.arange(0, n_points, 1), (shape.num_vertices, 1))[
            edge_bool
    ...
    face_bool = np.all(
        ...
        <= np.expand_dims(face_bounds, axis=2),
        axis=1,
    )  # <--- shape = (number_of_face_zones, number_of_points)
    if np.any(face_bool):
...
def shortest_displacement_to_surface(
    ...
    if np.any(vert_bool):
        ...
        v_points_used = np.tile(np.arange(0, n_points, 1), (n_verts, 1))[
            vert_bool
        ]  # Contains the indices of the points that hold True for vert_bool
        ...
        ) - points[v_points_used]  # Displacements between two points
    ...
    if np.any(face_bool):
        face_disp = point_to_face_displacement(
        ...
        min_disp_arr = np.concatenate(
            (min_disp_arr, np.expand_dims(face_disp, axis=1)), axis=1
        )

    disp_list_bool = np.argmin((np.linalg.norm(min_disp_arr, axis=2)), axis=1).reshape(
        n_points, 1, 1
    )
    true_min_disp = np.squeeze(
        np.take_along_axis(min_disp_arr, disp_list_bool, axis=1), axis=1
    )
...
def spheropolygon_shortest_displacement_to_surface(
    ...
    if np.any(vert_bool):
        ...
        # for spheropolygon
        vert_projection = np.squeeze(
            vert_disp
            - np.expand_dims(
                vert_disp @ (shape.normal / np.linalg.norm(shape.normal)), axis=1
            )
            * (shape.normal / np.linalg.norm(shape.normal)),
            axis=1,
        )
        v_projection_bool = np.linalg.norm(vert_projection, axis=1) > radius
        vert_projection[v_projection_bool] = (
            radius
            * vert_projection[v_projection_bool]
            / np.expand_dims(
                np.linalg.norm(vert_projection[v_projection_bool], axis=1), axis=1
            )
        )
    ...
    if np.any(edge_bool):
        ...
        # for spheropolygon
        edge_projection = np.squeeze(
            edge_disp
            - np.expand_dims(
                edge_disp @ (shape.normal / np.linalg.norm(shape.normal)), axis=1
            )
            * (shape.normal / np.linalg.norm(shape.normal)),
            axis=1,
        )
        e_projection_bool = np.linalg.norm(edge_projection, axis=1) > radius
        edge_projection[e_projection_bool] = (
            radius
            * edge_projection[e_projection_bool]
            / np.expand_dims(
                np.linalg.norm(edge_projection[e_projection_bool], axis=1), axis=1
            )
        )
